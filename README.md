...
安卓手机浏览器会显示触控优化的全屏关系面板。入口默认自动避开酒馆输入区，也可在面板中选择四角停靠；长按拖动入口会保存自定义位置。表单控件使用移动端尺寸，面板内部可独立滚动。
...
- `当前主 API`：沿用当前聊天使用的连接和模型，每次角色回复完成后额外调用一次。
- `独立连接档案`：从 SillyTavern Connection Manager 选择另一套 API、模型和预设。适合使用速度更快、价格更低、结构化输出更稳定的模型。

独立模式不会在本扩展中保存 API Key 或 API 地址。请先在 SillyTavern 的连接管理器中创建并测试连接档案，然后在关系面板选择该档案。扩展设置只保存连接档案 ID 和最大输出 token。

如果连接管理器被禁用、档案被删除或请求失败，本轮事件不会提交，旧关系状态保持不变。
...
- 事件抽取默认隔离酒馆预设和指令模板，防止角色扮演格式覆盖 `{"events":[...]}`。如需继承当前预设，可在“事件抽取 AI”中关闭隔离开关。
